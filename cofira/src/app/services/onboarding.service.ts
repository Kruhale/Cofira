...
export class OnboardingService {
  ...
  /* Progresos guardados por versiones viejas del asistente pueden traer números
     como texto ("74,5" con coma española): el backend los rechaza con 500 */
  private sanearNumero(valor: unknown): number | null {
  ...
  completeOnboarding(): Observable<OnboardingResponse> {
    ...
    data.heightCm = this.sanearNumero(data.heightCm);
    data.currentWeightKg = this.sanearNumero(data.currentWeightKg);
    data.targetWeightKg = this.sanearNumero(data.targetWeightKg);
    data.trainingDaysPerWeek = this.sanearNumero(data.trainingDaysPerWeek);
    data.mealsPerDay = this.sanearNumero(data.mealsPerDay);
    data.sleepHoursAverage = this.sanearNumero(data.sleepHoursAverage);
    data.sessionDurationMinutes = this.sanearNumero(data.sessionDurationMinutes);
