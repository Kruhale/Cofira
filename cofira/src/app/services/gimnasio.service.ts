...
export class GimnasioService {
  ...
  private construirSolicitudRutina(datosOnboarding: any): GenerarRutinaRequest {
    ...
    const solicitud: GenerarRutinaRequest = {
      ...
      deportesDisponibles: datosOnboarding.sports || [],
  ...
  // El backend resuelve el vídeo de técnica del ejercicio (primer resultado)
  obtenerVideoTecnica(nombreEjercicio: string): Observable<{ videoId: string }> {
    const consulta = encodeURIComponent(nombreEjercicio);
    return this.api.get<{ videoId: string }>("/rutinas-ejercicio/video-tecnica?ejercicio=" + consulta).pipe(
      catchError(() => of({ videoId: "" }))
    );
  }

  // Comentario del entrenador IA para la página de progreso
  obtenerMotivacion(): Observable<MotivacionIA | null> {
    return this.api.get<MotivacionIA>("/rutinas-ejercicio/motivacion").pipe(
      catchError(errorCapturado => {
        console.error("Error al obtener la motivacion:", errorCapturado);
        return of(null);
      })
    );
  }
  ...
  // Rutina de EJEMPLO para cuentas gratis: solo señales, sin tocar localStorage
  cargarRutinaDemo(rutina: RutinaGenerada): void {
    this.rutinaGenerada.set(rutina);
    this.ejerciciosPorDia.set(this.transformarRutinaAEjerciciosPorDia(rutina));
  }
