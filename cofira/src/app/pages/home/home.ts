...
export class Home {
  ...
  // Geometría del lienzo (viewBox 0 0 400 110): X regular por mes, Y proyectada.
  // A sangre (0-400): el plot comparte eje izquierdo con el resto de bandas.
  private readonly ejeX = [0, 80, 160, 240, 320, 400];
  private readonly lienzoIzquierda = 0;
  private readonly lienzoArriba = 16;
  private readonly lienzoAbajo = 88;
  private readonly lienzoPiso = 108;
  ...
  rutaArea = computed(
    () =>
      `${this.rutaLinea()} L400 ${this.lienzoPiso} L${this.lienzoIzquierda} ${this.lienzoPiso} Z`,
  );
  ...
  estadisticasMetrica = computed(() => {
    ...
    /* Se divide entre los 6 meses de la ventana (no entre los 5 tramos): es la
       cuenta que hace el lector con «Total · 6 meses» y evita que parezca errata */
    const ritmoMensual = cambioTotal / puntos.length;
  ...
  /* La etiqueta salta bajo la línea de objetivo cuando el último punto queda
     tan cerca por encima que la pisaría (caso cintura: 15 unidades) */
  objetivoEtiquetaY = computed(() => {
    const separacion = this.objetivoY() - this.puntoActual().y;
    if (separacion > 0 && separacion < 20) {
      return this.objetivoY() + 12;
    }
    return this.objetivoY() - 5;
  });

  // Valor de arranque de la serie, ya formateado para la regla de objetivo
  valorInicial = computed(() => {
    const metrica = this.metricaActiva();
    const primerPunto = metrica.puntos[0];
    return this.comaDecimal(primerPunto.valor);
  });
