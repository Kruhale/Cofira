...
import { HistorialEntrenamiento, MotivacionIA } from '../../models/gimnasio.model';
...
export class Seguimiento implements OnInit {
  ...
  /* true cuando el servidor confirma que la cuenta NO es PRO: aquí los datos
     son propios del usuario, solo cambia que se enseña el banner PRO */
  readonly esDemo = signal(false);

  /* Comentario del entrenador IA: como mucho 1 llamada al día (cache local) */
  readonly comentarioIA = signal<MotivacionIA | null>(null);
  readonly cargandoIA = signal(false);

  readonly ejerciciosEstaSemana = signal<number | null>(null);

  /* Anillos de la fila de métricas: 2π×r(44) del SVG compartido */
  private readonly CIRCUNFERENCIA_ANILLO = 276.5;

  readonly porcentajeKcalSemana = computed(() => {
    return Math.min(this.macrosConsumo().calorias.porcentaje, 100);
  });

  readonly adherenciaComidas = computed(() => {
    const resumen = this.consumoComidaService.resumenSemanal();
    if (!resumen || !resumen.totalComidas) {
      return 0;
    }
    const porcentaje = (resumen.comidasRegistradas / resumen.totalComidas) * 100;
    return Math.min(Math.round(porcentaje), 100);
  });

  readonly porcentajeAgua = computed(() => {
    const porcentaje = (this.aguaConsumida() / this.aguaObjetivo()) * 100;
    return Math.min(Math.round(porcentaje), 100);
  });

  calcularDashoffset(porcentaje: number): number {
    return this.CIRCUNFERENCIA_ANILLO * (1 - porcentaje / 100);
  }
  ...
  /* Polígono del relleno: la línea cerrada contra la base del gráfico (y=235) */
  readonly poligonoArea = computed(() => {
    const puntos = this.puntosGrafico();
    if (puntos.length < 2) {
      return '';
    }
    ...
    return `${this.polylinePoints()} ${ultimo.x},235 ${primero.x},235`;
  });
  ...
  ngOnInit(): void {
    this.suscripcionService.refrescarEstado().subscribe({
      next: (estado) => {
        this.esDemo.set(!estado.esPro);
        this.cargarComentarioIA(!estado.esPro);
      },
      error: () => {
        this.esDemo.set(true);
        this.cargarComentarioIA(true);
      },
    });

    this.cargarEjerciciosEstaSemana();
  ...
  /* En demo el comentario es fijo (no gastamos IA); en PRO se pide una vez al
     día y se cachea con la fecha para no quemar tokens en cada visita */
  private cargarComentarioIA(esCuentaDemo: boolean): void {
    if (esCuentaDemo) {
      this.comentarioIA.set(this.textos().entrenadorDemo);
      return;
    }

    const hoy = new Date().toISOString().slice(0, 10);
    try {
      const cacheGuardada = localStorage.getItem('cofira_motivacion_cache');
      if (cacheGuardada) {
        const cache = JSON.parse(cacheGuardada);
        if (cache.fecha === hoy && cache.comentario) {
          this.comentarioIA.set({ comentario: cache.comentario, consejo: cache.consejo });
          return;
        }
      }
    } catch {
      localStorage.removeItem('cofira_motivacion_cache');
    }

    this.cargandoIA.set(true);
    this.gimnasioService.obtenerMotivacion().subscribe({
      next: (motivacion) => {
        this.cargandoIA.set(false);
        if (!motivacion) {
          return;
        }
        this.comentarioIA.set(motivacion);
        localStorage.setItem(
          'cofira_motivacion_cache',
          JSON.stringify({ fecha: hoy, comentario: motivacion.comentario, consejo: motivacion.consejo }),
        );
      },
  ...
  private cargarEjerciciosEstaSemana(): void {
    this.gimnasioService.obtenerEstadisticas().subscribe({
      next: (estadisticas) => this.ejerciciosEstaSemana.set(estadisticas.ejerciciosCompletadosEstaSemana ?? 0),
      error: () => this.ejerciciosEstaSemana.set(null),
    });
  }
