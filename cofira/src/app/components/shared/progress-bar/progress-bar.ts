...
export class ProgressBar {
  ...
  /* El relleno cuadra con la etiqueta "Paso N de Total": en el paso N el avance
     es N/Total, así el último paso llega al 100% (antes se quedaba corto) */
  readonly progreso = computed(() => {
    const total = this.totalPasos();
    const actual = this.pasoActual();
    if (total <= 0) return 0;
    return Math.round(((actual + 1) / total) * 100);
  });
