...
export class Footer {
  ...
  /* En las páginas de la app (tras login) solo mostramos la barra legal: el
     bloque de marketing (CTA, newsletter, descarga de apps, comunidad) es para
     captar visitantes, no para usuarios que ya están dentro. */
  readonly minimo = input(false);
  ...
  constructor() {
    afterNextRender(() => {
      // El pie mínimo no pinta el pulso de comunidad: nada que animar.
      if (this.minimo()) {
        return;
      }
