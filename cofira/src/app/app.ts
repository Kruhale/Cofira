...
export class App {
  ...
  // Páginas de la app (ya dentro): el mega-footer de captación no pega, así que
  // ahí mostramos solo la barra legal mínima.
  // OJO: debe coincidir con las rutas protegidas por authGuard en app.routes.ts;
  // si añades una ruta de la app allí, añádela también aquí.
  private readonly rutasApp = [
    '/alimentacion',
    '/seguimiento',
    '/gimnasio',
    '/acceso-pro',
  ...
  protected readonly footerMinimo = computed(() =>
    this.rutasApp.some((ruta) => this.urlActual().startsWith(ruta)),
  );
}
