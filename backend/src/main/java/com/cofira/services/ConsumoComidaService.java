...
import com.cofira.repositories.RegistroComidaConsumidaRepository;
...
public class ConsumoComidaService {
    ...
    // Comidas al dia REALES del perfil: el denominador de la adherencia (antes era un 5 fijo)
    private int obtenerComidasPorDia(Usuario usuario) {
        return userProfileRepository.findByUsuarioId(usuario.getId())
                .map(perfil -> perfil.getMealsPerDay())
                .filter(comidas -> comidas > 0)
                .orElse(3);
    }
    ...
    public ResumenNutricionalRealDTO obtenerResumenRealDelDia(String username, LocalDate fecha) {
        ...
        return ResumenNutricionalRealDTO.builder()
                ...
                .totalComidas(obtenerComidasPorDia(usuario))
                .build();
    }
    ...
    public ResumenNutricionalRealDTO obtenerResumenSemanal(String username) {
        ...
        int totalComidasSemana = obtenerComidasPorDia(usuario) * 7;
