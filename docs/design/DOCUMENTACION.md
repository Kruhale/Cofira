...
He usado la estrategia hibrida emulated, la mayoría de los componentes usa ViewEncapsulation.Emulated para mantener los estilos encapsulados y evitar conflictos entre componentes. Sin embargo, para los estilos globales y utilidades he optado por ViewEncapsulation.None para que estos estilos puedan ser aplicados en todo el proyecto sin restricciones. Esta combinación me permite tener un control preciso sobre los estilos específicos de cada componente mientras mantengo una base de estilos globales accesible para todos los componentes. Lo he hecho asín para que cada componente pueda tener sus estilos protegidos sin tener problemas de especifidad, por la escalabilidad ya que de esta forma puedo reutilizar estilos globales en muchas partes y agilizar el proceso y por que es mucho mas mantenible al tener los estilos globales en un solo lugar.
...
# Sección 8: Accesibilidad Web

## 8.1 Fundamentos de accesibilidad

### Por qué es necesaria la accesibilidad web

La accesibilidad web es hacer que tu web la pueda usar cualquiera, tenga la discapacidad que tenga. Pero no es solo para personas con discapacidad, también ayuda a alguien que está usando el móvil con una mano o a alguien que está en un sitio ruidoso y necesita subtítulos. Además es obligatorio por ley cumplir WCAG 2.1 nivel AA.

### Los 4 principios WCAG 2.1 (POUR)

1. **Perceptible:** La información tiene que poder percibirse
...
2. **Operable:** Los componentes tienen que poder usarse con teclado
...
4. **Robusto:** Tiene que funcionar con tecnologías de asistencia
   - Ejemplo: Los SVGs decorativos que uso como iconos llevan `aria-hidden="true"` y los botones de toggle tienen `aria-pressed` para que VoiceOver anuncie si están activados o no
...
## 8.2 Componente multimedia implementado
...
## 8.3 Auditoría automatizada inicial
...
## 8.4 Análisis y corrección de errores
...
**Problema:** El skip link tenía muy poco contraste porque en `app.scss` tenía un `* { background-color }` que aplicaba un fondo claro a todo y me sobreescribía el fondo oscuro del skip link. Por lo que en vez de usar * para el fondo pues usé host para que solo hiciera efecto en el componente raiz y de esta forma no me sobreescribía el fondo del skip link.
...
He analizado todas las imágenes de la aplicación y todas tienen `alt` descriptivo. No tengo ninguna imagen decorativa con `alt=""` porque todas las imágenes que he usado aportan contenido. Los iconos SVG sí que los tengo todos con `aria-hidden="true"` para que los lectores de pantalla los ignoren.
...
Desconecté el ratón y navegué solo con teclado:

- [x] Llego a todos los enlaces y botones con Tab
- [x] El orden con Tab es lógico
- [x] Se ve bien qué elemento tiene el focus
...
| Las imágenes tienen descripciones adecuadas | Sí | Se leen con su alt |
| Los enlaces tienen textos descriptivos | Sí | Anuncian el nombre de la red social |
| El componente multimedia es accesible | Sí | La galería funciona bien |
...
![Chrome](../accesibilidad/capturas/chrome.png)

![Safari](../accesibilidad/capturas/safari.png)

![Brave](../accesibilidad/capturas/brave.png)

---

## 8.7 Resultados finales después de correcciones

Después de aplicar las correcciones volví a pasar las 3 herramientas:
...
Todas las páginas cumplen los criterios de nivel A y AA, no hay errores críticos aunque si es verdad que existe 1 error de crontaste en la página blog pero no me muestra ni me dice de donde viene dicho error de contraste. La navegación con teclado es completa y el contenido es comprensible. Además, la aplicación funciona bien con tecnologías de asistencia como lectores de pantalla como voiceover.


---

## 8.8 Conclusiones y reflexión
...
1. Añadir los estilos de burbuja de apple para navegadores antivos como safari.
2Hacer que el `aria-label` del botón de idiomas coincida con el texto visible
